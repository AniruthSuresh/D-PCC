...
def search_path(data_root, seq):
    # seq_dir = [os.path.join(data_root, s, 'velodyne_points', 'data') for s in seq if os.path.isdir(os.path.join(data_root, s))]
    seq_dir = [os.path.join(data_root, s, 'pcd_cam', 'data') for s in seq if os.path.isdir(os.path.join(data_root, s))]

    pcd_path = []
    for dir in seq_dir:
        cur_pcd_path = [os.path.join(dir, p) for p in os.listdir(dir) if p.endswith('.bin')]
        cur_pcd_path.sort()  # ensures ordered sequence
        pcd_path += cur_pcd_path
    return pcd_path
...
if __name__ == '__main__':
    ...
    dataset_args.data_root = dataset_args.data_root
